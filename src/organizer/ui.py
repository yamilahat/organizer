...
def rows_to_rules(rows: list[tuple]) -> list[dict]:
    ...
    for enabled, rtype, field, cat in rows:
        ...
        if str(rtype) == "glob" and str(field).strip():
            ...
            out.append(
                {"type": "ext", "exts": exts, "category": cat, "enabled": bool(enabled)}
            )
    return out
...
RULE_TEMPLATES = [
    ...
    (
        ".torrent",
        {"type": "ext", "exts": [".torrent"], "category": "archives", "enabled": True},
    ),
    (
...
def _autostart_launch_command(watch_root: str) -> str:
    _ = watch_root
    if getattr(sys, "frozen", False):
        return f'"{sys.executable}" --startup'
    return f'"{_pythonw()}" -m organizer.launcher --startup'
...
def enable_autostart(watch_root: str) -> None:
    ...
    launch_cmd = _autostart_launch_command(watch_root)

    content = (
        "@echo off\r\n"
        f'echo %DATE% %TIME% launching {launch_cmd} >> "{log}" 2>&1\r\n'
        # /B starts without a new window; START detaches so .cmd can exit immediately
        f'start "" /B {launch_cmd} >> "{log}" 2>&1\r\n'
        "exit /b 0\r\n"
    )
...
def main():
    ...
    style.configure(
        "Pill.TLabel", background="#e0f2fe", foreground="#0f172a", padding=(10, 4)
    )
    ...
    ttk.Label(hero, text="Organizer", style="HeroTitle.TLabel").grid(
        row=0, column=0, sticky="w"
    )
    ttk.Label(
        hero,
        text="Keep downloads tidy with smart rules and a background watcher.",
        style="HeroSubtitle.TLabel",
    ).grid(row=1, column=0, sticky="w", pady=(2, 0))
    ttk.Label(hero, textvariable=status_var, style="Pill.TLabel").grid(
        row=0, column=1, rowspan=2, sticky="e"
    )
    ttk.Label(hero, text=f"Config: {cfg_path}", style="Subtle.TLabel").grid(
    ...
    ysb = ttk.Scrollbar(
        outer, orient="vertical", command=canvas.yview, style="Thin.Vertical.TScrollbar"
    )
    ...
    ttk.Label(header, text="Watch Root", style="SectionHeading.TLabel").grid(
        row=0, column=0, sticky="w"
    )
    ...
    ttk.Button(
        ...
        command=lambda: watch_var.set(
            filedialog.askdirectory(title="Select folder to watch") or watch_var.get()
        ),
    ).grid(row=0, column=2, sticky="e")
    ttk.Label(
        header,
        text="Choose the folder to monitor for new files.",
        style="Subtle.TLabel",
    ...
    ttk.Button(
        ...
        text="Open Config",
        bootstyle=LINK,
        command=lambda: webbrowser.open(f"file:///{cfg_path}"),
    ).pack(side="left")
    ttk.Button(
        links,
        text="Open Journal Folder",
        bootstyle=LINK,
        command=lambda: os.startfile(os.path.dirname(JOURNAL_PATH)),
    ...
    dest_tree = ttk.Treeview(
        dest_frame, columns=dest_cols, show="headings", height=8, bootstyle=INFO
    )
    ...
    yscroll_dest = ttk.Scrollbar(
        dest_frame,
        orient="vertical",
        command=dest_tree.yview,
    ...
    def dest_add():
        cat = simpledialog.askstring(
            "Add Category", "Key (lowercase, no spaces):", parent=root
        )
        if not cat:
            return
        cat = cat.strip().lower()
    ...
    def dest_rename():
        ...
        new_cat = simpledialog.askstring(
            "Rename Category", "New key:", initialvalue=old_cat, parent=root
        )
        if not new_cat:
            return
    ...
    def dest_remove():
        ...
        if cat in ("archives", "installers", "docs"):
            if not messagebox.askyesno("Confirm", f"Remove default '{cat}'?"):
                return
    ...
    rules_frame = ttk.Labelframe(
        rules_tab, text="Rules (first match wins)", style="Card.TLabelframe"
    )
    ...
    rule_tree = ttk.Treeview(
        rules_frame, columns=rule_cols, show="headings", height=8, bootstyle=PRIMARY
    )
    ...
    def rule_add():
        rtype = simpledialog.askstring(
            "Rule Type", "Type (glob|ext):", parent=root, initialvalue="glob"
        )
        if not rtype:
            return
        ...
        field = simpledialog.askstring(
            "Pattern / Exts",
            "Glob pattern or space-separated extensions:",
            parent=root,
        ...
        cat = simpledialog.askstring(
            "Category",
            "Target category (must exist in Destinations):",
            parent=root,
            initialvalue="docs",
        )
        if not cat:
        ...
        if cat not in current_dest_dirs():
            messagebox.showwarning(
                "Unknown category",
                f"'{cat}' has no destination. Add it first in Destinations.",
            )
            return
    ...
    def rule_edit():
        ...
        en2 = simpledialog.askstring(
            "Enabled (true/false)", "Enabled:", initialvalue=str(bool(en)), parent=root
        ) or str(bool(en))
        rtype2 = (
            simpledialog.askstring(
                "Type (glob|ext)", "Type:", initialvalue=rtype, parent=root
            )
            or rtype
        )
        field2 = (
            simpledialog.askstring(
                "Pattern / Exts", "Value:", initialvalue=field, parent=root
            )
            or field
        )
        cat2 = (
            simpledialog.askstring(
                "Category", "Category:", initialvalue=cat, parent=root
            )
            or cat
        )
        ...
        rule_tree.item(
            sel[0],
            values=(str(en2).lower().startswith("t"), rtype2, field2.strip(), cat2),
        )
        ...
        for label, template in RULE_TEMPLATES:
            ...
            def _add(t=template):
                ...
                if cat and cat not in current_dest_dirs():
                    messagebox.showwarning(
                        "Unknown category", f"'{cat}' is not defined in Destinations."
                    )
                    return
                if t.get("type") == "ext":
                    rule_tree.insert(
    ...
    def rule_import():
        ...
        send_notification(
            "Organizer",
            f"Imported {len(rule_tree.get_children(''))} rules from {os.path.basename(path)}",
        )
    ...
    def rule_export():
        ...
        try:
            ...
            send_notification(
                "Organizer",
                f"Exported {len(rules_out)} rules to {os.path.basename(path)}",
            )
        except Exception as e:
    ...
    for btn in (
        ...
        ttk.Button(
            rule_toolbar, text="Enable/Disable", command=rule_toggle, bootstyle=WARNING
        ),
        ttk.Button(rule_toolbar, text="Up", command=lambda: rule_move(-1)),
        ttk.Button(rule_toolbar, text="Down", command=lambda: rule_move(1)),
        ttk.Button(
            rule_toolbar, text="Templates", command=rule_template_picker, bootstyle=INFO
        ),
    ...
    jrnl_container = ttk.Labelframe(
        content, text="Recent Activity (journal)", style="Card.TLabelframe"
    )
    jrnl_container.grid(
        row=3, column=0, columnspan=2, sticky="nsew", padx=12, pady=(0, 8)
    )
    ...
    def render_line(line: str) -> list[tuple[str, str]]:
        ...
        parts.append(
            (
                ...
                (
                    tag
                    if tag in ("planned", "finalized", "executed", "failed", "skip")
                    else ""
                ),
            )
        )
    ...
    ttk.Label(
        right, text="Watcher & notifications", style="SectionHeading.TLabel"
    ).grid(row=0, column=0, sticky="w", padx=8, pady=(6, 2))
    ttk.Label(right, textvariable=status_var, wraplength=260).grid(
    ...
    ttk.Checkbutton(right, text="Run in background", variable=background_var).grid(
        row=3, column=0, sticky="w", padx=8
    )

    notify_var = tk.BooleanVar(value=bool(raw.get("notify", False)))
    ttk.Checkbutton(right, text="Notifications", variable=notify_var).grid(
    ...
    def _tray_menu():
        return pystray.Menu(
            pystray.MenuItem(
                "Show Organizer", lambda icon, item: root.after(0, show_window)
            ),
            pystray.MenuItem(
                "Start watcher", lambda icon, item: root.after(0, start_watcher)
            ),
            pystray.MenuItem(
                "Stop watcher", lambda icon, item: root.after(0, stop_watcher)
            ),
            pystray.MenuItem(
                "Pause 30 minutes", lambda icon, item: root.after(0, tray_pause_30m)
            ),
            pystray.MenuItem(
                "Open config",
                lambda icon, item: root.after(
                    0, lambda: webbrowser.open(f"file:///{cfg_path}")
                ),
            ),
            pystray.MenuItem(
                "Open journal folder",
                lambda icon, item: root.after(
                    0, lambda: os.startfile(os.path.dirname(JOURNAL_PATH))
                ),
            ),
            ...
            pystray.MenuItem(
                "Quit",
                lambda icon, item: root.after(
                    0, lambda: (_stop_tray_icon(), root.destroy())
                ),
            ),
        )
    ...
    def ensure_tray_icon():
        ...
        if img is None:
            if not tray_warned:
                messagebox.showwarning(
                    "Tray unavailable", "Could not load tray icon image."
                )
                tray_warned = True
            return None
    ...
    def update_status():
        ...
        if running:
            status_var.set(
                f"Watcher: {run_txt} (PID {pid}) ֲ· {root_disp or 'set watch root'} ֲ· {auto_txt}"
            )
        else:
            status_var.set(f"Watcher: {run_txt} ֲ· {auto_txt}")
    ...
    def _start_watcher_worker(
        root_dir: str, cfg_new: dict, notify_on: bool, background_on: bool
    ):
        rec = read_pidfile()
        ...
        if getattr(sys, "frozen", False):
            cmd = [exe, "--watcher", "--watch", root_dir]
        else:
            cmd = [exe, "-m", "organizer.launcher", "--watcher", "--watch", root_dir]
        if notify_on:
            cmd.append("--notify")
        try:
            ...
            proc = subprocess.Popen(
                cmd, creationflags=flags, stdout=out, stderr=err, close_fds=True
            )
            write_pidfile(proc.pid, root_dir)
            send_notification("Organizer", f"Started (PID {proc.pid}).")
        except Exception as e:
            root.after(
                0, lambda: messagebox.showerror("Watcher", f"Failed to start: {e}")
            )
    ...
    def _stop_watcher_worker():
        ...
        try:
            subprocess.run(
                ["taskkill", "/PID", str(pid), "/T"], check=False, capture_output=True
            )
    ...
    def save_all():
        ...
        if running:
            if messagebox.askyesno(
                "Config saved", "Restart watcher now to apply changes?"
            ):
                restart_watcher()
            else:
    ...
    start_stop_btn = ttk.Button(
        btns, text="Start Watcher", command=start_stop_toggle, bootstyle=SUCCESS
    )
    ...
    if pystray is None or Image is None:
        ttk.Label(
            btns,
            text="Tray requires: pip install pystray Pillow",
            style="Muted.TLabel",
            wraplength=260,
